...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    if (!leave || leave.orgId !== user.orgId) return NextResponse.json({ error: 'not_found' }, { status: 404 });
    if (leave.status !== 'PENDING') return NextResponse.json({ error: 'not_pending' }, { status: 409 });

    const isHR = hasAnyRole(user, [ROLE.HR_MANAGER, ROLE.SUPER_ADMIN, ROLE.IT_MANAGER]);
    const canTeam =
      !!leave.departmentId && hasAnyRole(user, [ROLE.DIRECTOR, ROLE.MANAGER], leave.departmentId);
