...
export default function Page() {
  ...
  async function load() {
    setLoading(true);
    setErr(null);
    const res = await fetch('/api/approvals/leave?scope=team', { cache: 'no-store' });
    if (!res.ok) { setErr('Failed to load approvals'); setLoading(false); return; }
  ...
  return (
    <>
      <h1 className="text-xl font-semibold">Team approvals</h1>
      <p className="mt-1 text-sm text-gray-600">Pending leave requests from your department.</p>
