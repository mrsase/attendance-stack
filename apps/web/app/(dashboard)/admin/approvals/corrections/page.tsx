...
type Row = {
  ...
  user: { id: string; name: string|null; email: string|null; department?: { name?: string|null } };
};
...
export default function Page() {
  ...
  async function load() {
    setLoading(true);
    const res = await fetch('/api/approvals/corrections?scope=company', { cache: 'no-store' });
    if (res.ok) {
  ...
  return (
    <>
      <h1 className="text-xl font-semibold">Approve attendance corrections</h1>
      <p className="mt-1 text-sm text-gray-600">Company-wide pending corrections.</p>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-t">
                    ...
                    <td className="px-3 py-2">{(r.user as any).department?.name ?? '—'}</td>
